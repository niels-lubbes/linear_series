...
The method "get_linear_series()" is called from LinearSeries.get().
'''
...
def get_linear_series( deg, bp_tree ):
    ...
    #
    # Prevent ""NameError: global name 'LinearSeries' is not defined"
    # although it will give a warning that imports are not allowed at
    # this place.
    #
    from class_linear_series import *
    ...
    for ls in ls_lst:
        ...
        row_lst += [row]
    dprint( row_lst )
    dprint( matrix( row_lst ).parent(), '\n' + matrix( row_lst ).str() )

    #
    # Compute kernel.
    #
    kern = list( matrix( row_lst ).right_kernel().matrix() )
    dprint( matrix( kern ).parent(), '\n' + matrix( kern ).str() )
